...
function cleanBehavioralText(text: string): string {
  const p = pureText(text);
  if (!p) return p;
  const match = p.match(SIGN_OPENER);
  if (match) {
...
function ProfilePage() {
  ...
  const cleanPattern = pureText((synthesis as any).myPattern ?? (synthesis as any).coreEssence ?? "");
  const cleanStress  = pureText(synthesis.stressPattern ?? "");
