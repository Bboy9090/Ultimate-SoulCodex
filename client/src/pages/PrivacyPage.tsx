...
const sections = [
  ...
  ["2. How We Use Your Information", <ul><li>Generate and store profile features you request.</li><li>Provide compatibility analysis and requested AI-assisted guidance.</li><li>Perform online placement verification only when you explicitly opt in or invoke a server-backed feature that requires it.</li><li>Send notifications only when you opt in.</li><li>Verify premium entitlements and operate, secure, troubleshoot, and improve the service.</li></ul>],
  ["3. Data Storage & Security", <><p>Local-first profile creation is stored on your device for offline access and does not automatically upload your profile for verification. If you explicitly select online verification or use another server-backed feature, the information needed for that request may be stored in the configured production database and reconciled back into the same local profile.</p><p>Payment details are entered on Stripe&apos;s hosted checkout page. Soul Codex does not collect, transmit, log, or store card numbers, security codes, or expiration dates.</p></>],
  ["4. Third-Party Services", <ul><li><strong>OpenStreetMap/Nominatim:</strong> when you explicitly choose “Resolve place” and the location is not in Soul Codex&apos;s built-in list, the entered place text may be sent to Nominatim to look up coordinates. You can instead enter latitude, longitude, and timezone manually.</li><li><strong>AI providers:</strong> may process profile context and a question needed to generate a requested response.</li><li><strong>Stripe:</strong> hosts secure checkout and processes payment information when premium checkout is enabled.</li><li><strong>Infrastructure providers:</strong> host the app, database, and encrypted network traffic required to provide requested server-backed features.</li></ul>],
  ["5. Data Sharing", <p>We do not sell or rent your personal information for marketing. Data is shared only with service providers as needed to perform an action you request, operate or secure the service, or process payment.</p>],
  ["6. Your Choices and Rights", <ul><li><strong>Local-only creation:</strong> leave online verification off and your profile is created on the current device without a profile-verification upload.</li><li><strong>Online verification:</strong> opt in when you want supported placements checked by server-backed verification.</li><li><strong>Access:</strong> inspect profile data shown in the app.</li><li><strong>Local deletion:</strong> clear the saved profile and local application data from the current device.</li><li><strong>Server deletion:</strong> account holders may request deletion through Settings or support when account access is unavailable.</li><li><strong>Notifications:</strong> disable them through browser or device settings.</li></ul>],
      ...
      <main className="privacy-shell">
        <header className="privacy-hero">
          ...
          <div>
            ...
            <div className="privacy-meta"><span>Last updated August 14, 2026</span><span>No sale of personal data</span><span>Local-first profile storage</span></div>
          </div>
        </header>

        <section className="privacy-summary" aria-label="Privacy summary">
          <article><strong>On this device</strong><span>Your saved profile can remain available offline. Online verification is off unless you choose it.</span></article>
          <article><strong>On the server</strong><span>Only requested server-backed features may persist associated data in production storage.</span></article>
