 ...
 */
export function calculateMoralCompass(answers, lifePath, sunSign) {
    // Determine compass type from answers
    let compassType = 'Guardian';
    ...
    if (answers.neighborhoodType === 'close-knit') {
        if (compassType === 'Explorer')
            compassType = 'Builder';
        else if (compassType === 'Visionary')
            compassType = 'Protector';
    }
    else if (answers.neighborhoodType === 'diverse') {
        if (compassType === 'Guardian')
            compassType = 'Protector';
    }
    ...
    if (lifePath) {
        if ([1, 8].includes(lifePath))
            compassType = 'Visionary';
        else if ([2, 6, 9].includes(lifePath))
            compassType = 'Protector';
        else if ([4, 22].includes(lifePath))
            compassType = 'Guardian';
        else if ([3, 5].includes(lifePath))
            compassType = 'Builder';
        else if ([7, 11].includes(lifePath))
            compassType = 'Explorer';
    }
...
function generateFamilyInfluence(familyValues, compassType) {
    const influences = {
        traditional: {
...
function generateNeighborhoodInfluence(neighborhoodType, compassType) {
    const influences = {
        'close-knit': {
 ...
 */
export function calculateMoralCompassFromBirthData(lifePath, sunSign, moonSign) {
