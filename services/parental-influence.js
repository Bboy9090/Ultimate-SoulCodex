 ...
 */
export function calculateParentalInfluence(userSunSign, userMoonSign, fatherSign, motherSign) {
    ...
    const combinedInfluence = calculateCombinedInfluence(fatherInfluence, motherInfluence, userSunSign, userMoonSign);
...
function calculateParentInfluence(parentSign, userSunSign, userMoonSign, parentType) {
...
function generateDefaultInfluence(userSunSign, parentType) {
...
function calculateCombinedInfluence(father, mother, userSunSign, userMoonSign) {
...
function getSignCompatibility(sign1, sign2) {
    ...
    if (info1.modality === info2.modality)
        return 2; // Medium compatibility
    if (areOppositeSigns(sign1, sign2))
        return 1; // Challenging but growth-oriented
    return 1.5; // Neutral
}
function areOppositeSigns(sign1, sign2) {
    const opposites = {
        Aries: 'Libra',
        Taurus: 'Scorpio',
        Gemini: 'Sagittarius',
...
function generateInfluenceDescription(parentSign, userSunSign, userMoonSign, parentType, compatibility) {
...
function generateParentStrengths(parentSign, userSunSign, compatibility) {
    const parentInfo = getSignInfo(parentSign);
    const strengths = {
...
function generateParentChallenges(parentSign, userSunSign, compatibility) {
    const parentInfo = getSignInfo(parentSign);
    const challenges = {
...
function generateHowItShows(parentSign, userSunSign, parentType, compatibility) {
    const examples = {
        'Gemini-Virgo': {
...
function blendElements(element1, element2) {
    if (element1 === element2)
        return `Strong ${element1} influence`;
    const blends = {
...
function blendModalities(modality1, modality2) {
    if (modality1 === modality2)
        return `Strong ${modality1} energy`;
    const blends = {
...
function determineCompatibility(sign1, sign2) {
...
function generateDynamic(fatherSign, motherSign, userSunSign) {
...
function generateCombinedDescription(father, mother, userSunSign) {
    const userInfo = getSignInfo(userSunSign);
...
function generateHowItManifests(father, mother, userSunSign, elementalBlend) {
...
function generateLessons(father, mother, userSunSign, compatibility) {
    const lessons = [];
...
function extractInheritedTraits(father, mother, userSunSign) {
    const traits = [];
    // Inherit from father
...
function generateBalancingAct(father, mother, userSunSign) {
    const userInfo = getSignInfo(userSunSign);
...
function generateGrowthOpportunities(father, mother, userSunSign) {
    const opportunities = [];
    if (father.element !== mother.element) {
...
function generateRelationshipPatterns(father, mother, userSunSign) {
    const patterns = [];
    const compatibility = determineCompatibility(father.sign, mother.sign);
    if (compatibility === 'Harmonious') {
...
function getDefaultSignForElement(element) {
    const signs = {
        Fire: 'Aries',
        Earth: 'Taurus',
        Air: 'Gemini',
