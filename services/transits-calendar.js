 ...
 */
export function generateTransitsCalendar(profile, startDate, endDate) {
    const days = [];
    const allThemes = [];
    const peakDates = [];
    let totalTransits = 0;
 ...
 */
function generateRecommendations(transits, dominantTheme) {
 ...
 */
export function getUpcomingSignificantTransits(profile, days = 30) {
    const astrologyData = profile.astrologyData;
    const natalPlanets = extractNatalPositions(astrologyData);
    ...
    const uniqueTransits = Array.from(new Map(allTransits.map(t => [`${t.planet}-${t.natalPlanet}-${t.aspect}`, t])).values());
