 ...
 */
export function calculateElementalProfile(birthDate, lifePath, sunSign, moonSign, humanDesignType) {
    const scores = {
        Earth: 20,
        Water: 20,
        Fire: 20,
    ...
    // Life Path influence
    if (lifePath) {
        if ([2, 4, 6].includes(lifePath))
            scores.Earth += 12;
        if ([7, 9, 11].includes(lifePath))
            scores.Water += 12;
        if ([1, 3, 5].includes(lifePath))
            scores.Fire += 12;
        if ([5, 7, 9].includes(lifePath))
            scores.Air += 10;
        if ([8, 22, 33].includes(lifePath))
            scores.Metal += 12;
    }
    ...
    if (humanDesignType) {
        ...
        if (humanDesignType === 'Manifesting Generator') {
            scores.Fire += 10;
            scores.Earth += 5;
        }
    ...
    let excessElement = null;
    let deficientElement = null;
    if (maxScore > avgScore + 15)
        excessElement = sorted[0][0];
    if (minScore < avgScore - 10)
        deficientElement = sorted[4][0];
    // Calculate balance
...
function generateElementalInterpretation(primary, secondary, excess, deficient) {
...
function generateRemedies(excess, deficient, primary) {
    const remedies = [];
    // Add remedies for deficient element
    if (deficient) {
        const profile = ELEMENT_PROFILES[deficient];
        remedies.push({ category: 'movement', title: `Activate ${deficient}`, description: profile.helps[0], element: deficient, effect: 'increase' }, { category: 'ritual', title: `${deficient} Ritual`, description: profile.helps[1], element: deficient, effect: 'increase' });
    }
    ...
    if (excess) {
        ...
        remedies.push({ category: 'food', title: `Cool ${excess}`, description: `Balance with ${balancing}: ${profile.helps[0]}`, element: balancing, effect: 'balance' }, { category: 'rest', title: `Rest from ${excess}`, description: profile.helps[2] || profile.helps[1], element: balancing, effect: 'decrease' });
    }
    // Add primary element maintenance
    const primaryProfile = ELEMENT_PROFILES[primary];
    remedies.push({ category: 'environment', title: `Honor Your ${primary}`, description: primaryProfile.helps[3] || primaryProfile.helps[0], element: primary, effect: 'balance' });
    return remedies;
}
function generateWarnings(excess, deficient) {
    const warnings = [];
    if (excess) {
 ...
 */
export function generateSoulArchetype(name, lifePath, sunSign, moonSign, humanDesignType, enneagramType) {
    // Determine base archetype
    let archetypeKey = 'quiet-storm-architect';
    if (lifePath === 1 || lifePath === 8)
        archetypeKey = 'visionary-revolutionary';
    else if (lifePath === 2 || lifePath === 6 || lifePath === 9)
        archetypeKey = 'mystic-healer';
    else if (lifePath === 4 || lifePath === 22)
        archetypeKey = 'guardian-protector';
    else if (lifePath === 3 || lifePath === 5)
        archetypeKey = 'cosmic-creator';
    else if (lifePath === 7 || lifePath === 11)
        archetypeKey = 'quiet-storm-architect';
...
function generateFirstPersonBio(name, archetype, sunSign, moonSign) {
    const bios = {
...
function generateRomanticProfile(archetype, sunSign, moonSign, enneagramType) {
    const romanticProfiles = {
        'quiet-storm-architect': {
...
function generateDestinyReading(archetype, lifePath, humanDesignType) {
    return {
...
function generatePowerMode(archetype) {
    return {
        name: 'Power Mode',
...
function generateShadowMode(archetype) {
    return {
        name: 'Shadow Mode',
...
// ─────────────────────────────────────────────────────────────────────────────
export function getDailyElementalGuidance(primaryElement, date = new Date()) {
