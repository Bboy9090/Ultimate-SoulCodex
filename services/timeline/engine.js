...
export function getThemeSignals(topThemes) {
...
export function resolvePhase(signals) {
    if (signals.length === 0)
        return "Integration";
    const tally = {};
    ...
    let best = "Integration";
    let bestScore = -1;
    for (const [phase, score] of Object.entries(tally)) {
        if (score > bestScore) {
            bestScore = score;
            best = phase;
        }
...
export function nextPhaseFor(phase) {
