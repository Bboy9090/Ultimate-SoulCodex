...
function calculateDoshaFromBirthData(birthDate, sunSign, moonSign, enneagramType) {
    ...
    if (sunSign) {
        if (vataSign.includes(sunSign))
            vata += 15;
        else if (pittaSigns.includes(sunSign))
            pitta += 15;
        else if (kaphaSigns.includes(sunSign))
            kapha += 15;
    ...
    if (moonSign) {
        if (vataSign.includes(moonSign))
            vata += 10;
        else if (pittaSigns.includes(moonSign))
            pitta += 10;
        else if (kaphaSigns.includes(moonSign))
            kapha += 10;
...
export function calculateAyurveda(birthDate, astrologyData, personalityData) {
    const scores = calculateDoshaFromBirthData(birthDate, astrologyData?.sunSign, astrologyData?.moonSign, personalityData?.enneagram?.type);
    // Determine constitution
    const sorted = Object.entries(scores).sort((a, b) => b[1] - a[1]);
    const primary = sorted[0][0];
    const secondary = sorted[1][1] > 30 ? sorted[1][0] : undefined;
    let constitution;
    if (scores.vata > 40 && scores.pitta < 30 && scores.kapha < 30)
        constitution = "Vata";
    else if (scores.pitta > 40 && scores.vata < 30 && scores.kapha < 30)
        constitution = "Pitta";
    else if (scores.kapha > 40 && scores.vata < 30 && scores.pitta < 30)
        constitution = "Kapha";
    else if (scores.vata > 35 && scores.pitta > 35)
        constitution = "Vata-Pitta";
    else if (scores.pitta > 35 && scores.kapha > 35)
        constitution = "Pitta-Kapha";
    else if (scores.vata > 35 && scores.kapha > 35)
