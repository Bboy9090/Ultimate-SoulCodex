 ...
 */
export function calculateCongruenceScore(frequencyLogs, purposeStatement, lookbackDays = 30) {
    const now = new Date();
    ...
    const weightedScore = (frequencyTrend.score * 0.30) +
...
function calculateFrequencyTrend(logs) {
    ...
    const previousPeriodLogs = logs.filter(log => new Date(log.loggedAt) >= fourteenDaysAgo && new Date(log.loggedAt) < sevenDaysAgo);
