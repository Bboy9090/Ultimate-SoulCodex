...
export class SubscriptionService {
     ...
     */
    async confirmSubscription(checkoutSessionId) {
     ...
     */
    async createFallbackProfile(params) {
        const { userId, sessionId, email } = params;
    ...
    getPlanFromPriceId(priceId) {
        if (!priceId)
            return null;
        if (priceId === this.priceIds.weekly)
            return 'weekly';
        if (priceId === this.priceIds.monthly)
            return 'monthly';
        if (priceId === this.priceIds.yearly)
            return 'yearly';
        return null;
    }
}
