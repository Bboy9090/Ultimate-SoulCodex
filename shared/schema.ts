...
// Durable purchase authority for web and native stores. These records are
// account-scoped; profile flags remain compatibility fields, not entitlement
// authority.
...
export const storeTransactionEvents = pgTable("store_transaction_events", {
  ...
  billingSubjectId: varchar("billing_subject_id").notNull()
    .references(() => billingSubjects.id, { onDelete: "restrict" }),
  ...
  providerTransactionUnique: uniqueIndex("store_events_provider_transaction_uidx")
    .on(table.provider, table.environment, table.externalTransactionId),
...
export const entitlementGrants = pgTable("entitlement_grants", {
  ...
  sourceEventId: varchar("source_event_id").notNull()
    .references(() => storeTransactionEvents.id, { onDelete: "restrict" }),
  ...
  subjectCapabilityUnique: uniqueIndex("entitlement_grants_subject_capability_uidx")
    .on(table.billingSubjectId, table.capability),
