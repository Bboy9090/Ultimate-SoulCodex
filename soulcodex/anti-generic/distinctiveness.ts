/**
 * Distinctiveness Scoring Model — Phase 2
 *
 * Scores each candidate statement on six dimensions and returns
 * a final 0-1 score. Statements below REJECT_THRESHOLD are dropped.
 *
 * Weights:
 *   behavioral_specificity  0.25  — does it describe a concrete action/pattern?
 *   distinguishing_power    0.25  — how uniquely does it match THIS user's signals?
 *   contradiction_depth     0.15  — does it contain surface/hidden tension?
 *   phase_relevance         0.15  — does it match the top active themes?
 *   action_usefulness       0.10  — does it lead to something you could actually do?
 *   language_originality    0.10  — does it avoid banned frames + repetitive openers?
 */

import type { Statement } from "./statements/index";
import { totalPenaltyScore } from "./banned-language";
...
export function scoreStatement(
  ...
  // Hard reject: no behavioral verb
  ...
  // Top 3 themes get full credit; tail themes get less
  ...
  let langOrig = 1;
  langOrig -= banPenalty * 0.8;                             // partial-ban penalty
  if (hasGenericOpener(s.text)) langOrig -= 0.3;
  // Repetition: penalise if same 3-word opener already used
