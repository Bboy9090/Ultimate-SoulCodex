import { getPlanetSignInterpretation, getHouseInterpretation, getKarmicInterpretation } from "./interpretations";
...
function createBirthTime(birthData) {
    try {
        ...
        const resolvedTimezone = resolveTimezone(birthData.timezone, parseFloat(birthData.latitude.toString()), parseFloat(birthData.longitude.toString()));
...
function calculateMidheaven(birthTime, longitude) {
    const lst = calculateLocalSiderealTime(birthTime, longitude);
    const lstDegrees = lst * 15;
    const obliquity = 23.4397;
    const oblRad = obliquity * Math.PI / 180;
...
export function calculateAstrology(birthData) {
...
export function getTarotBirthCards(birthDate) {
