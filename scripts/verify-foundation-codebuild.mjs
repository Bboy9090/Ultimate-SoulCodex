...
check("PRIVACY-03", "Local-first UI explains verification boundary", files.localFirst.includes("Online astronomy and verified Human Design core calculation happen only when you explicitly choose them.") && files.localFirst.includes("Leave this off to keep profile creation entirely on-device."));
...
check("PRIVACY-06", "Astronomy and Human Design verification are evidence-only", files.verificationRoute.includes('app.post("/api/verification/profile"') && files.verificationRoute.includes("persistedProfile: false") && files.verificationRoute.includes("aiGeneration: false") && !/^import .*storage/im.test(files.verificationRoute) && !/^import .*openai/im.test(files.verificationRoute));
