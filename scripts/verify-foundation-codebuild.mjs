...
check("ASTRO-03", "Saved profiles require the current verified full-natal and Human Design migration", files.reconciliation.includes("CURRENT_ASTROLOGY_VERIFICATION_VERSION = 7") && files.reconciliation.includes("hasVerifiedFullNatalChart") && files.reconciliation.includes('humanDesignData?.status !== "verified"'));
