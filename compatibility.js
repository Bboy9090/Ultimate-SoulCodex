...
function calculateAstrologyCompatibility(profile1, profile2) {
    ...
    const karmicConnection = checkKarmicConnection(astro1.northNode?.sign, astro1.southNode?.sign, astro2.northNode?.sign, astro2.southNode?.sign);
    // If we have synastry results, use them to enhance the score and aspects
    let astroScore = Math.round(sunScore * 0.25 +
        moonScore * 0.30 +
        risingScore * 0.15 +
        venusMarsScore * 0.20 +
...
function calculateSignCompatibility(sign1, sign2) {
    ...
    if (sign1 === sign2)
        return 100; // Same sign - deep understanding
...
function getSunCompatibilityDescription(sign1, sign2, score) {
    if (score >= 90)
        return `${sign1} and ${sign2} share the same element, creating natural harmony in core values and life approach. You "get" each other instinctively.`;
    if (score >= 80)
        return `${sign1} and ${sign2} complement each other beautifully. Your different approaches to life actually enhance the relationship.`;
    if (score >= 70)
        return `${sign1} and ${sign2} create magnetic tension - opposite but deeply attracted. This pairing requires conscious effort but offers profound growth.`;
    if (score >= 55)
        return `${sign1} and ${sign2} have some friction in their core approaches, but this challenge can spark growth if both are willing to understand each other's perspective.`;
...
function getMoonCompatibilityDescription(sign1, sign2, score) {
    if (score >= 90)
        return `Emotional languages align perfectly. You intuitively understand each other's feelings and needs without words.`;
    if (score >= 80)
        return `Your emotional natures complement each other. One provides what the other needs for emotional security.`;
    if (score >= 70)
        return `Emotional connection requires conscious effort, but the magnetic pull is strong. Different needs can actually balance each other.`;
    if (score >= 55)
        return `You express emotions differently, which can lead to misunderstandings. Communication about feelings is key.`;
...
function getRisingCompatibilityDescription(sign1, sign2, score) {
    if (score >= 90)
        return `You approach life in similar ways. First impressions were likely instant recognition and comfort.`;
    if (score >= 80)
        return `Your life approaches complement each other. You're attracted to how the other presents themselves to the world.`;
    if (score >= 70)
        return `Different styles create intrigue. You're fascinated by each other's unique approach to life.`;
...
function getVenusMarsDescription(score) {
    if (score >= 85)
        return `Intense magnetic attraction! The chemistry is palpable and the passion runs deep. This is a rare level of physical and romantic compatibility.`;
    if (score >= 70)
        return `Strong romantic chemistry with good balance between attraction and shared values in love.`;
    if (score >= 55)
        return `Moderate attraction with potential to grow. Initial spark may take time to develop into lasting passion.`;
...
function calculateNumerologyCompatibility(profile1, profile2) {
    ...
    const numerologyScore = Math.round(lifePathScore * 0.50 +
...
function calculateNumberCompatibility(num1, num2) {
    ...
    if (compatiblePairs[num1.toString()]?.includes(num2))
        return 85;
    if (compatiblePairs[num2.toString()]?.includes(num1))
        return 85;
    return 60;
}
function getLifePathCompatibilityDescription(num1, num2, score) {
    if (score === 100)
        return `Same Life Path (${num1}) - You share the same soul mission and understand each other's journey intimately.`;
    if (score >= 85)
        return `Life Paths ${num1} and ${num2} are naturally harmonious. Your life purposes support and enhance each other.`;
...
function getExpressionCompatibilityDescription(score) {
    if (score >= 85)
        return `You express yourselves in complementary ways, making communication natural and flowing.`;
...
function getPersonalityCompatibilityDescription(score) {
    if (score >= 85)
        return `Your outer personalities mesh well, creating ease in social situations and first impressions.`;
...
function calculateHumanDesignCompatibility(profile1, profile2) {
    ...
    const hdScore = Math.round(typeScore * 0.30 +
        authorityScore * 0.25 +
        centerScore * 0.25 +
...
function getTypeInteractionDescription(type1, type2, score) {
    if (type1 === 'Generator' && type2 === 'Projector')
        return `Powerful pairing! ${type2}s are here to guide ${type1}s' abundant energy, creating mutual fulfillment.`;
    if (type1 === 'Projector' && type2 === 'Generator')
        return `Powerful pairing! ${type1}s are here to guide ${type2}s' abundant energy, creating mutual fulfillment.`;
    if (score >= 85)
        return `${type1} and ${type2} create natural energetic harmony. Your interaction feels easeful and supportive.`;
    if (score >= 70)
        return `${type1} and ${type2} can work well together with mutual understanding of each other's strategy.`;
...
function calculateAuthorityCompatibility(auth1, auth2) {
    ...
    if (emotionalAuthorities.includes(auth1) && emotionalAuthorities.includes(auth2))
        return 85;
    if (sacralAuthorities.includes(auth1) && sacralAuthorities.includes(auth2))
        return 85;
    return 70;
}
function getAuthorityDescription(auth1, auth2, score) {
    if (score >= 85)
        return `Similar decision-making processes create understanding in how you both navigate choices.`;
...
function calculateCenterDynamics(centers1, centers2) {
    ...
    if (complementaryRatio >= 0.4 && complementaryRatio <= 0.6)
        return 90;
    if (complementaryRatio >= 0.3 && complementaryRatio <= 0.7)
        return 80;
    return 70;
}
function getCenterDynamicsDescription(score) {
    if (score >= 85)
        return `Excellent balance of defined and undefined centers. You complement each other's energetic strengths and wisdom.`;
...
function calculatePersonalityCompatibility(profile1, profile2) {
    ...
    const personalityScore = Math.round(enneagramScore * 0.40 +
        mbtiScore * 0.35 +
        cognitiveScore * 0.25);
    return {
...
function calculateEnneagramCompatibility(type1, type2) {
    ...
    if (compatibility[type1]?.includes(type2))
        return 85;
    return 65;
}
function getEnneagramDescription(type1, type2, score) {
    if (!type1 || !type2)
        return 'Enneagram data incomplete';
    if (score && score >= 85)
        return `Type ${type1} and Type ${type2} create natural harmony. Your core motivations align and support each other's growth.`;
...
function getMBTIDescription(type1, type2, score) {
    if (!type1 || !type2)
        return 'MBTI data incomplete';
    if (score && score >= 85)
        return `${type1} and ${type2} share similar cognitive approaches. You understand how each other thinks and processes information.`;
    if (score && score >= 70)
        return `${type1} and ${type2} have complementary perspectives. Your differences create balance and mutual learning.`;
...
function calculateCognitiveFunctionAlignment(type1, type2) {
    ...
    if (dominantFunction1 === dominantFunction2)
        return 90;
    // Complementary functions (N-S, T-F)
    if ((dominantFunction1.includes('N') && dominantFunction2.includes('S')) ||
        (dominantFunction1.includes('S') && dominantFunction2.includes('N')))
        return 75;
    if ((dominantFunction1.includes('T') && dominantFunction2.includes('F')) ||
        (dominantFunction1.includes('F') && dominantFunction2.includes('T')))
        return 75;
    return 70;
}
function getCognitiveDescription(score) {
    if (score >= 85)
        return `Your cognitive functions align beautifully, making intellectual connection natural and stimulating.`;
...
function calculateVedicCompatibility(profile1, profile2) {
    ...
    if (vedic1.vedicSun === vedic2.vedicSun)
        score += 15;
    if (vedic1.vedicMoon === vedic2.vedicMoon)
        score += 15;
    return {
...
function calculateChineseCompatibility(profile1, profile2) {
    ...
    let score = 70;
    const isCompatible = compatiblePairs.some(pair => (pair[0] === year1 && pair[1] === year2) || (pair[1] === year1 && pair[0] === year2));
    if (year1 === year2)
        score = 85; // Same animal
    else if (isCompatible)
        score = 90;
    // Element compatibility
    if (chinese1.element === chinese2.element)
        score += 5;
    return {
        score: Math.min(100, score),
...
function calculateAyurvedaCompatibility(profile1, profile2) {
    ...
    if (dosha1 === 'Vata' && dosha2 === 'Kapha')
        score = 85;
    else if (dosha1 === 'Kapha' && dosha2 === 'Vata')
        score = 85;
    else if (dosha1 === 'Pitta' && dosha2 === 'Kapha')
        score = 80;
    else if (dosha1 === 'Kapha' && dosha2 === 'Pitta')
        score = 80;
    else if (dosha1 === dosha2)
        score = 75; // Same dosha understands each other
    return {
        score,
        description: score >= 80
...
function calculateGeneKeysCompatibility(profile1, profile2) {
    ...
    let score = 70;
    // Life Work alignment
    if (gk1.lifeWork?.gift === gk2.lifeWork?.gift)
        score += 10;
    // Evolution paths
    if (gk1.evolution?.gift === gk2.evolution?.gift)
        score += 10;
    return {
        score: Math.min(100, score),
...
function calculateIChingCompatibility(profile1, profile2) {
    ...
    if (Math.abs(hexNum1 - hexNum2) === 1)
        score = 85;
    else if (hexNum1 === hexNum2)
        score = 90;
    return {
        score,
...
function calculateMayanCompatibility(profile1, profile2) {
    ...
    if (mayan1.daySign?.name === mayan2.daySign?.name)
        score = 85;
    // Tone harmony
...
function calculateChakraCompatibility(profile1, profile2) {
    ...
    if (dominant1 === dominant2)
        score = 80;
    else if ((dominant1 === 'Heart' && dominant2 === 'Throat') || (dominant1 === 'Throat' && dominant2 === 'Heart'))
        score = 90;
    return {
        score,
...
function calculateSacredGeometryCompatibility(profile1, profile2) {
    ...
    let score = 75;
    if (sg1.primaryShape === sg2.primaryShape)
        score = 85;
    return {
        score,
...
function calculateRunesCompatibility(profile1, profile2) {
    ...
    let score = 75;
    if (runes1.birthRune === runes2.birthRune)
        score = 85;
    return {
        score,
...
function calculateBiorhythmsCompatibility(profile1, profile2) {
    ...
    if (bio1.physicalPeakDay === bio2.physicalPeakDay)
        score += 10;
    return {
        score: Math.min(100, score),
...
function calculateArabicPartsCompatibility(profile1, profile2) {
    ...
    if (ap1.fortuneSign === ap2.fortuneSign)
        score += 10;
    return {
        score: Math.min(100, score),
...
function calculateFixedStarsCompatibility(profile1, profile2) {
    ...
    let score = 75;
    if (fs1.primaryStar === fs2.primaryStar)
        score = 90;
    return {
        score,
...
function calculateKabbalahCompatibility(profile1, profile2) {
    ...
    let score = 75;
    if (kab1.treePath === kab2.treePath)
        score = 85;
    return {
        score,
...
function calculateTarotCompatibility(profile1, profile2) {
    ...
    if (tarot1.personality?.number === tarot2.personality?.number)
        score = 90;
    else if (tarot1.soul?.number === tarot2.soul?.number)
        score = 85;
    return {
        score,
        description: score >= 85
...
// Overall Synthesis
function createCompatibilitySynthesis(astro, num, hd, pers, moralCompass) {
    const strengths = [];
    const challenges = [];
    const growthOpportunities = [];
    // Identify strengths
    if (astro.moonCompatibility.score >= 85)
        strengths.push('Deep emotional understanding');
    if (astro.venusMarsChemistry.score >= 85)
        strengths.push('Strong romantic chemistry');
    if (astro.karmicConnection.hasConnection)
        strengths.push('Karmic soul connection');
    if (num.lifePathCompatibility.score >= 85)
        strengths.push('Aligned life purposes');
    if (hd.channelConnections.count > 0)
        strengths.push('Energetic activation and chemistry');
    if (pers.enneagramCompatibility.score >= 85)
        strengths.push('Compatible core motivations');
    if (moralCompass && moralCompass.score >= 80)
        strengths.push('Shared values and ethical alignment');
    // Identify challenges
    if (astro.sunCompatibility.score < 70)
        challenges.push('Different core values require understanding');
    if (num.score < 70)
        challenges.push('Different life rhythms and expressions');
    if (hd.typeInteraction.score < 70)
        challenges.push('Different energetic strategies');
    if (pers.mbtiCompatibility.score < 70)
        challenges.push('Different communication and thinking styles');
    ...
    if (avgScore >= 85)
        relationshipType = 'Soul Mate Connection';
    else if (avgScore >= 75)
        relationshipType = 'Highly Compatible Partnership';
    else if (avgScore >= 65)
        relationshipType = 'Growth-Oriented Relationship';
    else
        relationshipType = 'Challenging but Transformative Connection';
    // Long-term potential
...
function calculateMoralCompassCompatibility(profile1, profile2) {
    const moral1 = profile1.moralCompassData;
    const moral2 = profile2.moralCompassData;
    if (!moral1 || !moral2) {
...
export function calculateCompatibility(profile1, profile2) {
    ...
    const spiritualScore = Math.round((vedic.score + chinese.score + ayurveda.score + geneKeys.score +
    ...
    const overallScore = Math.round(astrology.score * 0.25 +
