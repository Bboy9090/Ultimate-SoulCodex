...
export const TRANSMUTATION_TECHNIQUES = {
    'Pluto': {
        planet: 'Pluto',
...
export function getTransmutationTechnique(planet) {
    return TRANSMUTATION_TECHNIQUES[planet] || null;
}
// Get all techniques
export function getAllTransmutationTechniques() {
    return Object.values(TRANSMUTATION_TECHNIQUES);
}
// Get recommended technique based on active transits
export function getRecommendedTechnique(activeTransits) {
...
export function getActiveTransmutationTechniques(activeTransits) {
    if (!activeTransits || activeTransits.length === 0)
        return [];
    const techniques = [];
