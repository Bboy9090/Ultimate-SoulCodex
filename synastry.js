...
function calculateHouseOverlays(person1Planets, person2HouseCusps, // Array of 12 house cusp longitudes for person2
person2Name) {
    if (!person2HouseCusps || person2HouseCusps.length < 12)
        return [];
    const overlays = [];
 ...
 */
export function calculateDetailedSynastry(chart1, chart2) {
    const allAspects = [];
    ...
    const chemistryScore = Math.round(goldenAspects.reduce((sum, asp) => sum + asp.score, 0) / Math.max(goldenAspects.length, 1));
    const commitmentScore = Math.round(diamondAspects.reduce((sum, asp) => sum + asp.score, 0) / Math.max(diamondAspects.length, 1) || 50);
    const growthScore = Math.round((allAspects.filter(a => a.impact === 'tension').reduce((sum, asp) => sum + asp.score, 0) /
        Math.max(allAspects.filter(a => a.impact === 'tension').length, 1)) || 60);
    // Overall score weighted by importance
    const overallScore = Math.round(chemistryScore * 0.35 +
        commitmentScore * 0.40 +
        growthScore * 0.25);
    // Generate summary
    const soulMateIndicators = [];
    if (goldenAspects.length >= 3)
        soulMateIndicators.push('Multiple Golden Tier aspects - strong soul connection');
    if (diamondAspects.length >= 2)
        soulMateIndicators.push('Saturn aspects present - long-term staying power');
    if (fatedAspects.length >= 1)
        soulMateIndicators.push('Karmic/fated connection indicators');
    ...
    const strengths = [];
    if (goldenAspects.length > 0)
        strengths.push(`${goldenAspects.length} harmonious soul mate aspects`);
    if (diamondAspects.length > 0)
        strengths.push(`${diamondAspects.length} commitment-building Saturn aspects`);
    if (chemistryScore >= 80)
        strengths.push('Exceptional natural chemistry and attraction');
    const challenges = [];
    const tensionAspects = allAspects.filter(a => a.impact === 'tension');
    if (tensionAspects.length > 5)
        challenges.push('Multiple challenging aspects requiring conscious work');
    if (commitmentScore < 60)
        challenges.push('May lack long-term stability indicators');
    if (goldenAspects.length === 0)
        challenges.push('Limited natural harmony - will require more effort');
    const relationshipType = overallScore >= 85 ? 'Soul Mate Connection' :
