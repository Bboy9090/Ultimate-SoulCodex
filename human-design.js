...
function degreeToGateAndLine(degree) {
    ...
    if (positionInGate < 0)
        positionInGate += 360; // handle wrap-around
...
function hasMotorToThroatConnection(channels, centers) {
    ...
    for (const channel of definedChannels) {
        ...
        if (connectsCenters.includes("Throat") &&
            connectsCenters.some((c) => isMotorCenter(c) && centers[c]?.defined)) {
            return true;
        }
    }
    return false;
...
function calculateType(centers, channels) {
    ...
    if (sacralDefined) {
        ...
        const hasSacralThroatConnection = channels.some(ch => ch.defined &&
...
function calculateDefinition(centers, channels) {
    ...
    for (const channel of definedChannels) {
        ...
        if (!centerConnections[center1])
            centerConnections[center1] = new Set();
        if (!centerConnections[center2])
            centerConnections[center2] = new Set();
...
export function calculateHumanDesign(birthData) {
    ...
    const resolvedTimezone = resolveHDTimezone(birthData.timezone, parseFloat(birthData.latitude), parseFloat(birthData.longitude));
...
export function getHumanDesignInterpretation(hdData) {
    ...
    return `As a ${type}, ${typeDescriptions[type] || typeDescriptions.Generator} Your strategy is "${strategy}" and your authority is "${authority}". Your profile ${profile} indicates your life theme and how you interact with the world. This combination creates your unique energetic blueprint for navigating life authentically.`;
}
