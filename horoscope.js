...
import { calculatePersonalDayNumber, getMoonPhase } from './daily-context';
...
function calculatePlanetLongitude(planet, date) {
    if (planet === 'Moon') {
...
export function calculateCurrentPlanets(date = new Date()) {
    const planets = [];
...
function getAlignmentInterpretation(p1, p2, aspect) {
    const key1 = `${p1}-${p2}`;
    const key2 = `${p2}-${p1}`;
...
export function calculateAlignments(planets) {
    const alignments = [];
...
export function calculatePersonalTransitsFromProfile(profile, date = new Date()) {
    if (!profile.astrologyData)
        return [];
...
async function generateAIHoroscope(profile, planets, alignments, personalTransits, moonPhase, personalDayNumber) {
...
function generateFallbackHoroscope(sunSign, moonSign, moonPhase, personalDayNumber, alignments, personalTransits) {
    const dayThemes = {
...
function getDateKeyInTimezone(now, timezone) {
    if (!timezone)
...
export async function generateDailyHoroscope(profile) {
    const now = new Date();
