...
function calculateCycle(daysSinceBirth, period) {
    ...
    let phase;
    if (Math.abs(value) < 5)
        phase = "Critical"; // Near zero crossing
    else if (value > 0)
        phase = "High";
    else
        phase = "Low";
    ...
    if (daysUntilPeak < 0)
        daysUntilPeak += period;
    return { value, phase, daysUntilPeak: Math.round(daysUntilPeak) };
}
export function calculateBiorhythms(birthDate, currentDate = new Date()) {
    ...
    return {
        ...
        overall: {
            ...
            interpretation: `Your overall biorhythm energy is at ${overallEnergy}%. ${overallEnergy > 50
