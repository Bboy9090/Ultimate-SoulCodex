...
export function buildTodayCard(horoscopeData, profile, codexSynthesis) {
    ...
    const decisionStyle = profile?.signals?.decisionStyle ??
        profile?.userInputs?.decisionStyle ??
        "";
    const rawConf = profile?.confidence ?? profile?.meta?.confidence ?? null;
    const badge = rawConf?.badge === "verified" || rawConf?.badge === "partial" || rawConf?.badge === "unverified"
        ? rawConf.badge
    ...
    const label = (rawConf?.label ??
        (badge === "verified" ? "Verified" : badge === "partial" ? "Partial" : "Unverified"));
    const reason = rawConf?.reason ??
        (badge === "verified"
    ...
    const aiAssuranceNote = rawConf?.aiAssuranceNote ??
        (badge === "verified"
    ...
    let focus = `Personal Day ${dayNum} — a ${moonPrefix.toLowerCase()} phase for ${topTheme.replace(/_/g, " ")}. ${topTransit ? topTransit.description?.slice(0, 80) + "." : "Stay in your build lane today."}`;
    if (focus.length > 120)
        focus = focus.slice(0, 117) + "…";
...
export function buildTodayCardSvg(card, format) {
    const W = 1080;
    ...
    const confColor = card.confidenceLabel === "Verified" ? "#22c55e" :
        card.confidenceLabel === "Partial" ? "#f59e0b" : "#6b7280";
    const bullets = (items, icon, color, x, y, maxW) => {
