...
import { birthDataSchema, enneagramAssessmentSchema, mbtiAssessmentSchema, signupSchema, loginSchema } from "@soulcodex/db";
...
import { buildPosterSvg } from "../../server/posterSvg";
...
import { calculateElementalProfile, generateSoulArchetype } from "../../services/elemental-medicine";
import { calculateMoralCompassFromBirthData } from "../../services/moral-compass";
...
import { getAllPrompts, getPromptById } from "../../services/journaling";
...
import { generateProfilePDF, generateCompatibilityPDF, renderPDF } from "../../services/pdf-generator";
...
export async function registerRoutes(app) {
    ...
    app.post('/api/stripe/webhook', (req, res, next) => {
        ...
        try {
            ...
            let result = 'success';
            let metadata = { eventType: event.type, receivedAt: new Date().toISOString() };
            try {
                switch (event.type) {
    ...
    app.post("/api/soul-archetype", async (req, res) => {
        try {
                    ...
                    if (resolvedGeo) {
                        ...
                        if (!validatedBirthData.timezone) {
                            try {
                                ...
                                if (tzs.length > 0)
                                    validatedBirthData.timezone = tzs[0];
                            }
                            catch (e) {
            ...
            const hasCompleteData = !!(validatedBirthData.birthTime &&
            ...
            try {
                if (astrologyData && numerologyData) {
                    elementalMedicineData = calculateElementalProfile(validatedBirthData.birthDate, numerologyData?.lifePath, astrologyData.sunSign, astrologyData.moonSign, humanDesignData?.type);
                }
            }
            catch (error) {
            ...
            try {
                if (numerologyData && astrologyData) {
                    soulArchetypeData = generateSoulArchetype(validatedBirthData.name, numerologyData?.lifePath || 1, astrologyData.sunSign, astrologyData.moonSign, humanDesignData?.type, undefined // enneagramType
                    );
                }
            }
            catch (error) {
            ...
            try {
                if (validatedBirthData.moralCompassAnswers &&
                    ...
                    moralCompassData = calculateMoralCompass(validatedBirthData.moralCompassAnswers, numerologyData?.lifePath, astrologyData?.sunSign);
                }
                else {
                    moralCompassData = calculateMoralCompassFromBirthData(numerologyData?.lifePath, astrologyData?.sunSign, astrologyData?.moonSign);
                }
            }
            catch (error) {
            ...
            try {
                if (astrologyData) {
                    parentalInfluenceData = calculateParentalInfluence(astrologyData.sunSign, astrologyData.moonSign, validatedBirthData.fatherSign, validatedBirthData.motherSign);
                }
            }
            catch (error) {
    ...
    app.post("/api/profiles", async (req, res) => {
        try {
            ...
            const hasCompleteData = !!(birthData.birthTime &&
            ...
            if (hasCompleteData && astrologyData) {
                ...
                sabianSymbolsData = await runWithTimeoutAndTiming("Sabian Symbols", TIMEOUT_VALUES.SABIAN_SYMBOLS, () => calculateSabianSymbols(sunLongitude, moonLongitude, ascendantLongitude), null);
            }
            else {
            ...
            if (hasCompleteData && astrologyData) {
                try {
                    const ascendantLongitude = astrologyData.houses[0].degree;
                    asteroidsData = calculateAsteroids(birthData.birthDate, birthData.birthTime, birthData.timezone, ascendantLongitude);
                }
                catch (error) {
            ...
            if (hasCompleteData && astrologyData) {
                try {
                    const ascendantLongitude = astrologyData.houses[0].degree;
                    arabicPartsData = calculateArabicParts(ascendantLongitude, astrologyData.planets.sun.degree, astrologyData.planets.moon.degree, astrologyData.planets.venus.degree, astrologyData.planets.jupiter.degree, astrologyData.planets.saturn.degree, true // isDayBirth - simplified
                    );
                }
                catch (error) {
            ...
            try {
                if (astrologyData && numerologyData) {
                    elementalMedicineData = calculateElementalProfile(birthData.birthDate, numerologyData?.lifePath, astrologyData.sunSign, astrologyData.moonSign, humanDesignData?.type);
                }
            }
            catch (error) {
            ...
            try {
                if (numerologyData && astrologyData) {
                    soulArchetypeData = generateSoulArchetype(birthData.name, numerologyData?.lifePath || 1, astrologyData.sunSign, astrologyData.moonSign, humanDesignData?.type, undefined // enneagramType - can be added later
                    );
                }
            }
            catch (error) {
            ...
            try {
                if (astrologyData) {
                    parentalInfluenceData = calculateParentalInfluence(astrologyData.sunSign, astrologyData.moonSign, birthData.fatherSign, birthData.motherSign);
                }
            }
            catch (error) {
            ...
            try {
                if (birthData.moralCompassAnswers &&
                    ...
                    const { calculateMoralCompass } = await import("./services/moral-compass");
                    moralCompassData = calculateMoralCompass(birthData.moralCompassAnswers, numerologyData?.lifePath, astrologyData?.sunSign);
                }
                else {
            ...
            const biography = await runWithTimeoutAndTiming("Gemini Biography", TIMEOUT_VALUES.GEMINI_BIOGRAPHY, () => generateBiography({
                name: birthData.name,
            ...
            const dailyGuidance = await runWithTimeoutAndTiming("Gemini Daily Guidance", TIMEOUT_VALUES.GEMINI_DAILY_GUIDANCE, () => generateDailyGuidance({
                name: birthData.name,
                archetypeTitle: baseArchetypeData.title,
                astrologyData,
                numerologyData,
    ...
    app.get("/api/profiles/:id", async (req, res) => {
        try {
            ...
            const profileHasCompleteData = !!(profile.birthTime &&
    ...
    app.get("/api/profiles/:id/unified", async (req, res) => {
        try {
            ...
            const archData = profile.archetypeData;
            const elemData = profile.elementalMedicineData;
            const moralData = profile.moralCompassData;
    ...
    app.get("/api/profiles/:id/soul-codex-v1", async (req, res) => {
        try {
            ...
            if (!profile)
                return res.status(404).json({ message: "Profile not found" });
            const tone_mode = req.query.tone_mode === "clean" || req.query.tone_mode === "deep" || req.query.tone_mode === "raw"
                ? req.query.tone_mode
                : "clean";
    ...
    app.get("/api/profiles/:id/rituals", async (req, res) => {
        try {
            ...
            if (dailyInsightsRecord?.insightsData) {
                const insightsData = dailyInsightsRecord.insightsData;
            ...
            const hasCompleteData = !!(profile.birthTime &&
                profile.birthLocation &&
                profile.timezone &&
                profile.latitude &&
                profile.longitude);
                ...
                try {
                    ...
                    if (natalPositions && Object.keys(natalPositions).length > 0) {
                        ...
                        if (transmutation.activeTransits.length > 0) {
                            ...
                            const highIntensityTransits = transmutation.activeTransits.filter((t) => t.intensity === 'high');
    ...
    app.post("/api/profiles/:id/enneagram", async (req, res) => {
        try {
            ...
            const baseArchetypeData = synthesizeArchetype(profile.astrologyData, profile.numerologyData, updatedPersonalityData);
    ...
    app.get("/api/compatibility/:profile1Id/:profile2Id", async (req, res) => {
        try {
            ...
            const fullData = compatibility.compatibilityData;
            if (isPremium) {
    ...
    app.post("/api/push/test", async (req, res) => {
        try {
            ...
            const results = await Promise.all(subscriptions.map(sub => sendNotification(sub, { type: validated.type, context: validated.context })));
    ...
    app.post("/api/push/send-install-prompt", async (req, res) => {
        try {
            ...
            const results = await Promise.all(subscriptions.map(sub => sendAppInstallPrompt(sub, isMobile)));
            // Clean up expired subscriptions (maintain correct indices)
            const expiredSubs = [];
            for (let i = 0; i < results.length; i++) {
                if (results[i].expired) {
    ...
    app.post("/api/push/send-premium-upsell", async (req, res) => {
        try {
            ...
            const results = await Promise.all(subscriptions.map(sub => sendPremiumUpsell(sub)));
            // Clean up expired subscriptions (maintain correct indices)
            const expiredSubs = [];
            for (let i = 0; i < results.length; i++) {
                if (results[i].expired) {
    ...
    app.post("/api/push/broadcast", async (req, res) => {
        try {
            ...
            const results = await sendBulkNotifications(activeSubscriptions, { type: validated.type, context: validated.context }, async (expiredSub) => {
    ...
    app.get("/api/progressions/solar-return", async (req, res) => {
        try {
            ...
            const returnYear = parseInt(req.query.year) || new Date().getFullYear();
    ...
    app.get("/api/progressions/lunar-return", async (req, res) => {
        try {
            ...
            const returnDate = req.query.date ? new Date(req.query.date) : new Date();
    ...
    app.post("/api/poster/render", async (req, res) => {
        try {
            const width = Math.min(parseInt(req.query.width) || 2048, 4096);
    ...
    app.post("/api/today/render", async (req, res) => {
        try {
            ...
            if (!card)
                return res.status(400).json({ ok: false, error: "card data required" });
            const svg = buildTodayCardSvg(card, format);
    ...
    app.post("/api/codex30/generate", async (req, res) => {
        try {
            ...
            const clarityReport = await validateAndClean(narrative, (p) => generateText({ model: "gemini-2.0-flash", prompt: p, temperature: 0.85 }));
    ...
    app.get("/api/profiles/:profileId/lifemap", async (req, res) => {
        try {
            ...
            const numData = profile.numerologyData;
            const archData = profile.archetypeData;
            const confData = profile.confidence;
...
function buildFallbackNarrative(codename, anchors, themes, strengths, triggers) {
