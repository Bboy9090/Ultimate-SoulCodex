...
export function runBlandnessFilter(text) {
    const lower = text.toLowerCase();
    const bannedFound = BANNED_PHRASES.filter((phrase) => lower.includes(phrase.toLowerCase()));
    const missingElements = MUST_HAVE_PATTERNS.filter(({ pattern }) => !pattern.test(text)).map(({ label }) => label);
    return {
