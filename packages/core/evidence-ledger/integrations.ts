...
import { calcLifePath, calcExpression, calcSoulUrge, calcPersonality, normalizeNumerologyName } from '../compute/numerology.js';
...
export function calcPersonalityWithEvidence(
  ...
  const normalizedName = normalizeNumerologyName(fullName);
  const consonantCount = [...normalizedName].filter((letter) => !'AEIOU'.includes(letter)).length;
