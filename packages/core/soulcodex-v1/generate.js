...
import { buildCodexReadingBadges } from "../compute/confidence";
...
function signElement(sign) {
    ...
    if (!s)
        return null;
    if (["aries", "leo", "sagittarius"].includes(s))
        return "Fire";
    if (["taurus", "virgo", "capricorn"].includes(s))
        return "Earth";
    if (["gemini", "libra", "aquarius"].includes(s))
        return "Air";
    if (["cancer", "scorpio", "pisces"].includes(s))
        return "Water";
    return null;
}
function signMode(sign) {
    const s = (sign ?? "").toLowerCase();
    if (!s)
        return null;
    if (["aries", "cancer", "libra", "capricorn"].includes(s))
        return "Cardinal";
    if (["taurus", "leo", "scorpio", "aquarius"].includes(s))
        return "Fixed";
    if (["gemini", "virgo", "sagittarius", "pisces"].includes(s))
        return "Mutable";
    return null;
}
function uniq(items) {
...
function buildConfidenceMatrix(profile) {
    ...
    const numerology = birthDate ? "verified" : "unverified";
    // Astrology + Human Design both require time+geo to be fully verified; reuse computed badge.
    const astrology = badge;
    const human_design = badge;
    // Overall is a coarse quality signal (high when we have verified chart layer + date).
    const overall = astrology === "verified" && human_design === "verified" && numerology === "verified"
        ? "high"
...
function buildCoreSystem(profile) {
...
function defaultIdentitySummary(profile, core) {
    ...
    const one_line = oneLineParts.length > 0
...
function buildIdentityFromEngine(profile, core, engine) {
    const arch = profile?.archetype?.name ??
        profile?.archetypeData?.archetype ??
        profile?.archetypeData?.title ??
        profile?.archetypeData?.name ??
        "Soul Codex";
    const coreIdentityPool = engine.statements_by_section?.core_identity ?? [];
    ...
    const system_translation = topTraits.length > 0
...
function bulletsFromText(text, max = 5) {
...
function sectionFromEngine(title, statements) {
...
function buildSectionsFromEngine(engine) {
    const by = engine.statements_by_section;
    return {
...
function buildDailyGuidanceFromEngine(engine) {
...
export function generateSoulCodexOutputV1(input) {
    ...
    const engine = runSoulCodexEngine({
        toneMode,
        astrology: (input.profile?.astrologyData ?? input.profile?.astrology ?? null),
        human_design: (input.profile?.humanDesignData ?? input.profile?.humanDesign ?? null),
        numerology: (input.profile?.numerologyData ?? input.profile?.numerology ?? null),
    });
