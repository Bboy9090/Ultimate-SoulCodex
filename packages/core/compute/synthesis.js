...
function buildCoreEssence(s) {
    ...
    if (s.sunSign)
        parts.push(`- When it's time to take action, your ${s.sunSign} energy takes the wheel unquestioned.`);
    parts.push("\nWHY THIS MATTERS:");
...
function buildRelationshipPattern(s) {
    const parts = [];
    const social = {
...
function buildPowerMode(s) {
    ...
    partsOut.push("\nHOW THIS SHOWS UP IN YOUR LIFE:");
    if (partsInner.length > 0)
        partsOut.push(`- Whenever you lack the ability to ${partsInner[0]}, you probably feel unmotivated or stuck.`);
